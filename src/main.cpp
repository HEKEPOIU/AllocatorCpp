...
int main() {
	...
	auto ptr_all = alloc_none_zero(allocator, BUFFER_SIZE);
	std::print("{}", ptr_all.len());

	for (int i = 0; i < ptr_all.len(); i++) {
		std::print("{}", ptr_all[i]);
	}
	std::print("\n\n");
	...
	auto int_arr = alloc(allocator, 100, int);

	for (auto& i : int_arr) { i = 3; }
	for (auto i : int_arr) { std::print("{}", i); }
}